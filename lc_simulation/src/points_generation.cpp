...
int main(int argc, char **argv)
{
    ...
    // generate points
    lc_simulation::PointsSimulation<double> points_simulation(y_base, start_point, end_point, mid_point, points_num);

    std::vector<Eigen::Vector3d> points = points_simulation.getPoints();
    lc_core::outputPoints(FLAGS_ref_points_file, points);
    ...
    lc_core::outputPoints(FLAGS_output_file, points);
