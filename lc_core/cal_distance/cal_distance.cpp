...
void Cal_distance::excu_line_point()
{
    ...
    LOG(INFO) << "After excluding points from filtered.pcd, remaining points in cloud_raw: " << cloud_raw_filtered->size() ;
}
...
void Cal_distance::visual()
{
    ...
    for (const auto& pair : matched_points) {
        pcl::PointXYZRGB p1;
        p1.x = pair.first.x; p1.y = pair.first.y; p1.z = pair.first.z;
        p1.r = 255; p1.g = 0; p1.b = 0; // red cloud_final
        
        pcl::PointXYZRGB p2;
        p2.x = pair.second.x; p2.y = pair.second.y; p2.z = pair.second.z;
        p2.r = 0; p2.g = 0; p2.b = 255; // blue cloud_line
  ...
  // cloud_final to white for compare
    pcl::PointCloud<pcl::PointXYZRGB>::Ptr white_cloud_final(new pcl::PointCloud<pcl::PointXYZRGB>);
    ...
    LOG(INFO) << "Filtered cloud saved to filtered_cloud.pcd with " << white_cloud_final->size() << " white points." ;
}
...
{
    std::string file_line_pcd = "/home/gct/LC-CurveModel/data/wangan_0630/tempfinal_line_points.pcd";
    std::string file_raw_pcd = "/home/gct/LC-CurveModel/data/wangan_0630/extracted_points_0630.pcd";
    std::string matched_points_file = "/home/gct/LC-CurveModel/data/matched_points.pcd";
    std::string filtered_cloud_file = "/home/gct/LC-CurveModel/data/filtered_cloud.pcd";
    
    std::vector<std::string> output_files;
    output_files.push_back(matched_points_file);
    output_files.push_back(filtered_cloud_file);
    Cal_dist_paragram cal_dist_paragram(file_line_pcd,file_raw_pcd,output_files);
    Cal_distance cal_distance(cal_dist_paragram);

    cal_distance.load_pcd_file();

    cal_distance.excu_line_point();
    cal_distance.cloud_tunnel_filter();
    
    cal_distance.get_clostest_points();
