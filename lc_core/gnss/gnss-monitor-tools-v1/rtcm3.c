...
static int test_staid(rtcm_t *rtcm, int staid)
{
    ...
    if (rtcm->staid==0||rtcm->obsflag) {
        ...
        /*trace(5,"rtcm3 %d staid invalid id=%d %d\n",type,staid,rtcm->staid);*/
...
static int decode_type1002(rtcm_t *rtcm)
{
    ...
    for (j=0;j<nsat&&rtcm->obs.n<MAXOBS&&i+74<=rtcm->len*8;j++) {
        ...
        if (!(sat=satno(sys,prn))) {
            /*trace(5,"rtcm3 1002 satellite number error: prn=%d\n",prn);*/
            continue;
        }
...
static int decode_type1004(rtcm_t *rtcm)
{
    ...
    for (j=0;j<nsat&&rtcm->obs.n<MAXOBS&&i+125<=rtcm->len*8;j++) {
        ...
        if (!(sat=satno(sys,prn))) {
            /*trace(5,"rtcm3 1004 satellite number error: sys=%d prn=%d\n",sys,prn);*/
            continue;
        }
        tt=timediff(rtcm->obs.data[0].time,rtcm->time);
        if (rtcm->obsflag||fabs(tt)>1E-9) {
...
static int decode_type1005(rtcm_t *rtcm)
{
    ...
    if (i+140==rtcm->len*8) {
        ...
        /*trace(5,"rtcm3 1005 length error: len=%d\n",rtcm->len);*/
        return -1;
    }
    if (rtcm->outtype) {
        msg=rtcm->msgtype+strlen(rtcm->msgtype);
...
static int decode_type1006(rtcm_t *rtcm)
{
    ...
    if (i+156<=rtcm->len*8) {
        ...
        /*trace(5,"rtcm3 1006 length error: len=%d\n",rtcm->len);*/
        return -1;
    }
    if (rtcm->outtype) {
        msg=rtcm->msgtype+strlen(rtcm->msgtype);
...
static int decode_type1007(rtcm_t *rtcm)
{
    ...
    if (i+28+8*n<=rtcm->len*8) {
        ...
        /*trace(5,"rtcm3 1007 length error: len=%d\n",rtcm->len);*/
        return -1;
    }
    if (rtcm->outtype) {
        msg=rtcm->msgtype+strlen(rtcm->msgtype);
...
static int decode_type1008(rtcm_t *rtcm)
{
    ...
    if (i+36+8*(n+m)<=rtcm->len*8) {
        ...
        /*trace(5,"rtcm3 1008 length error: len=%d\n",rtcm->len);*/
        return -1;
    }
    if (rtcm->outtype) {
        msg=rtcm->msgtype+strlen(rtcm->msgtype);
...
static int decode_type1010(rtcm_t *rtcm)
{
    ...
    for (j=0;j<nsat&&rtcm->obs.n<MAXOBS&&i+79<=rtcm->len*8;j++) {
        ...
        cnr1 =getbitu(rtcm->buff,i, 8); i+= 8;
        if (!(sat=satno(sys,prn))) {
            /*trace(5,"rtcm3 1010 satellite number error: prn=%d\n",prn);*/
            continue;
        }
...
static int decode_type1012(rtcm_t *rtcm)
{
    ...
    for (j=0;j<nsat&&rtcm->obs.n<MAXOBS&&i+130<=rtcm->len*8;j++) {
        ...
        cnr2 =getbitu(rtcm->buff,i, 8); i+= 8;
        if (!(sat=satno(sys,prn))) {
            /*trace(5,"rtcm3 1012 satellite number error: sys=%d prn=%d\n",sys,prn);*/
            continue;
        }
        if (!rtcm->nav.glo_fcn[prn-1]) {
            rtcm->nav.glo_fcn[prn-1]=fcn-7+8; /* fcn+8 */
...
static int decode_type1019(rtcm_t *rtcm)
{
    ...
    if (i+476<=rtcm->len*8) {
        ...
        /*trace(5,"rtcm3 1019 length error: len=%d\n",rtcm->len);*/
        return -1;
    }
    if (prn>=40) {
        sys=SYS_SBS; prn+=80;
    ...
    if (!(sat=satno(sys,prn))) {
        /*trace(5,"rtcm3 1019 satellite number error: prn=%d\n",prn);*/
        return -1;
    }
    eph.sat=sat;
...
static int decode_type1020(rtcm_t *rtcm)
{
    ...
    if (!(sat=satno(sys,prn))) {
        /*trace(5,"rtcm3 1020 satellite number error: prn=%d\n",prn);*/
        return -1;
    }
    /*trace(5,"decode_type1020: prn=%d tk=%02.0f:%02.0f:%02.0f svh=%d\n",prn,tk_h,tk_m,tk_s,bn);*/
...
static int decode_type1029(rtcm_t *rtcm)
{
    ...
    if (i+60<=rtcm->len*8) {
        ...
        /*trace(5,"rtcm3 1029 length error: len=%d\n",rtcm->len);*/
        return -1;
    }
    if (i+nchar*8>rtcm->len*8) {
        /*trace(5,"rtcm3 1029 length error: len=%d nchar=%d\n",rtcm->len,nchar);*/
        return -1;
    } 
...
static int decode_type1033(rtcm_t *rtcm)
{
    ...
    if (i+60+8*(n+m+n1+n2+n3)<=rtcm->len*8) {
        ...
        /*trace(5,"rtcm3 1033 length error: len=%d\n",rtcm->len);*/
        return -1;
    }
    if (rtcm->outtype) {
        msg=rtcm->msgtype+strlen(rtcm->msgtype);
...
static int decode_type1041(rtcm_t *rtcm)
{
    ...
    if (i+482-12<=rtcm->len*8) {
        ...
        /*trace(5,"rtcm3 1041 length error: len=%d\n",rtcm->len);*/
        return -1;
    }
    ...
    if (!(sat=satno(sys,prn))) {
        /*trace(5,"rtcm3 1041 satellite number error: prn=%d\n",prn);*/
        return -1;
    }
    eph.sat=sat;
    eph.week=adjgpsweek(week);
...
static int decode_type1044(rtcm_t *rtcm)
{
    ...
    if (i+473<=rtcm->len*8) {
        ...
        /*trace(5,"rtcm3 1044 length error: len=%d\n",rtcm->len);*/
        return -1;
    }
    ...
    if (!(sat=satno(sys,prn))) {
        /*trace(5,"rtcm3 1044 satellite number error: prn=%d\n",prn);*/
        return -1;
    }
    eph.sat=sat;
    eph.week=adjgpsweek(week);
...
static int decode_type1045(rtcm_t *rtcm)
{
    ...
    if (i+484<=rtcm->len*8) {
        ...
        /*trace(5,"rtcm3 1045 length error: len=%d\n",rtcm->len);*/
        return -1;
    }
    ...
    if (!(sat=satno(sys,prn))) {
        /*trace(5,"rtcm3 1045 satellite number error: prn=%d\n",prn);*/
        return -1;
    }
    ...
    eph.iodc=eph.iode;
    /*trace(5, "decode_type1045: prn=%d iode=%d toe=%.0f svh=%02x\n", prn, eph.iode, eph.toes,eph.svh);*/
...
static int decode_type1046(rtcm_t *rtcm)
{
    ...
    if (i+492<=rtcm->len*8) {
        ...
        /*trace(5,"rtcm3 1046 length error: len=%d\n",rtcm->len);*/
        return -1;
    }
    if (rtcm->outtype) {
        msg=rtcm->msgtype+strlen(rtcm->msgtype);
    ...
    if (!(sat=satno(sys,prn))) {
        /*trace(5,"rtcm3 1046 satellite number error: prn=%d\n",prn);*/
        return -1;
    }
    ...
    eph.iodc=eph.iode;
    /*trace(5, "decode_type1046: prn=%d iode=%d toe=%.0f svh=%02x\n", prn, eph.iode, eph.toes, eph.svh);*/
...
static int decode_type1042(rtcm_t *rtcm)
{
    ...
    if (i+499<=rtcm->len*8) {
        ...
        /*trace(5,"rtcm3 1042 length error: len=%d\n",rtcm->len);*/
        return -1;
    }
    ...
    if (!(sat=satno(sys,prn))) {
        /*trace(5,"rtcm3 1042 satellite number error: prn=%d\n",prn);*/
        return -1;
    }
    eph.sat=sat;
...
static int decode_ssr1(rtcm_t *rtcm, int sys, int subtype)
{
    ...
    for (j=0;j<nsat&&i+121+np+ni+nj<=rtcm->len*8;j++) {
        ...
        if (!(sat=satno(sys,prn))) {
            /*trace(5,"rtcm3 %d satellite number error: prn=%d\n",type,prn);*/
            continue;
        }
    ...
    for (j=0;j<nsat&&i+5+np<=rtcm->len*8;j++) {
        ...
        for (k=0;k<nbias&&i+19<=rtcm->len*8;k++) {
            ...
            if (sigs[mode]) {
                ...
                /*trace(5,"rtcm3 %d not supported mode: mode=%d\n",type,mode);*/
            }
        }
        if (!(sat=satno(sys,prn))) {
            /*trace(5,"rtcm3 %d satellite number error: prn=%d\n",type,prn);*/
                     ...
                     int *idx)
{
    ...
    for (i=nex=0;i<n;i++) {
        if (ex[i]==0) ;
            ...
            /*trace(5,"rtcm msm: no space in obs data sys=%d code=%d\n",sys,code[i]);*/
            idx[i]=-1;
        }
#if 0 /* for debug */
        /*trace(5,"sig pos: sys=%d code=%d ex=%d idx=%d\n",sys,code[i],ex[i],idx[i]);*/
#endif
    }
}
                         ...
                         const int *ex, const int *half)
{
    ...
    for (i=0;i<h->nsig;i++) {
        ...
        if (code[i]!=CODE_NONE) {
            ...
            /*trace(5,"rtcm3 %d: unknown signal id=%2d\n",type,h->sigs[i]);*/
        }
    }
    /*trace(5,"rtcm3 %d: signals=%s\n",type,msm_type);*/
    ...
    for (i=j=0;i<h->nsat;i++) {
        ...
        if ((sat=satno(sys,prn))) {
            ...
            /*trace(5,"rtcm3 %d satellite error: prn=%d\n",type,prn);*/
        }
        fcn=0;
        if (sys==SYS_GLO) {
                           ...
                           msm_h_t *h, int *hsize)
{
    ...
    *h=h0;
    ...
    if (h->nsat*h->nsig>64) {
        /*trace(5,"rtcm3 %d number of sats and sigs error: nsat=%d nsig=%d\n",
              type,h->nsat,h->nsig);*/
        return -1;
    }
    if (i+h->nsat*h->nsig>rtcm->len*8) {
        /*trace(5,"rtcm3 %d length error: len=%d nsat=%d nsig=%d\n",type,
...
static int decode_msm4(rtcm_t *rtcm, int sys)
{
    ...
    if (i+h.nsat*18+ncell*48>rtcm->len*8) {
        /*trace(5,"rtcm3 %d length error: nsat=%d ncell=%d len=%d\n",type,h.nsat,
              ncell,rtcm->len);*/
        return -1;
    }
...
static int decode_type1230(rtcm_t *rtcm)
{
    ...
    if (i+20>=rtcm->len*8) {
        /*trace(5,"rtcm3 1230: length error len=%d\n",rtcm->len);*/
        return -1;
    }
    staid=getbitu(rtcm->buff,i,12); i+=12;
...
static int decode_type4076(rtcm_t *rtcm)
{
    ...
    if (i+3+8>=rtcm->len*8) {
        /*trace(5,"rtcm3 4076: length error len=%d\n",rtcm->len);*/
        return -1;
    }
...
extern int decode_rtcm3(rtcm_t *rtcm, rnxopt_t* opt, FILE* ofp)
{
    double tow;
