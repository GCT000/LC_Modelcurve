...
const prcopt_t prcopt_default={ /* defaults processing options */
    PMODE_STATIC,0,2,SYS_GPS|SYS_GAL|SYS_GLO|SYS_CMP,   /* mode,soltype,nf,navsys */
    15.0 * D2R,{{1,1},{{ 32,32,32,33,33,34,34,34,34 },{32,32,32,33,33,34,34,34,34},{0,0,0,0,0,0,0,0,0}}},           /* elmin,snrmask */
    0,3,0,0,1,1,                /* sateph,modear,glomodear,gpsmodear,bdsmodear,arfilter */
    30,0,3,8,9,0,             /* maxout,minlock,minfixsats,minholdsats,mindropsats,minfix */
    1,1,1,0,0,                  /* armaxiter,estion,esttrop,dynamics,tidecorr */
    1,0,1,0,0,                  /* niter,codesmooth,intpref,sbascorr,sbassatsel */
    0,1,                        /* rovpos,refpos */
    {200.0,250.0,300.0},        /* eratio[] */
    {0.0,0.004,0.003,0.0,1.0,52.0,0.0,0.0}, /* err[-,base,el,bl,dop,snr_max,snr,rcverr] */
    {30.0,0.03,0.3},            /* std[] */
    {1E-4,1E-3,1E-4,0.3,0.7,0.0}, /* prn[] */
    5E-12,                      /* sclkstab */
    {3.0,0.3,0.0,0.1,0.05,0.0,0.0,0.0}, /* thresar */
    15.0 * D2R,15.0 * D2R,0.05,0,             /* elmaskar,elmaskhold,thresslip,thresdop, */
	0.001,0.01,30.0,15.0,30.0,     /* varholdamb,gainholdamb,maxtdif,maxinno,maxgdop */
    {0},{0},{0},                /* baseline,ru,rb */
    {"",""},                    /* anttype */
    {{0}},{{0}},{0},            /* antdel,pcv,exsats */
    0,0,RANDOM_COM                         /* maxaveep,initrst */
};
const solopt_t solopt_default={ /* defaults solution output options */
    SOLF_ENU,TIMES_GPST,1,3,    /* posf,times,timef,timeu */
    0,1,1,0,0,0,0,              /* degf,outhead,outopt,outvel,datum,height,geoid */
    1,0,0,                      /* solstatic,sstat,trace */
    {0.0,0.0},                  /* nmeaintv */
...
extern int satexclude(int sat, double var, int svh, const prcopt_t *opt)
{
    ...
    if (svh) {
        /*trace(5,"unhealthy satellite: sat=%3d svh=%02X\n",sat,svh);*/
        return 1;
    }
    if (var>MAX_VAR_EPH) {
        /*trace(5,"invalid ura satellite: sat=%3d ura=%.2f\n",sat,sqrt(var));*/
        return 1;
    }
    return 0;
}
...
static lock_t lock_table;       /* lock for table */

static void traceswap(FILE* fp_trace)
{
...
extern void traceopen()
{
    initlock(&lock_trace);
}
extern void tableopen()
{
    initlock(&lock_trace);
}
extern void traceclose(FILE* fp_trace)
{
    if (fp_trace&&fp_trace!=stderr) fclose(fp_trace);
    fp_trace=NULL;
    file_trace[0]='\0';
}
extern void tableclose(FILE* fp_table)
{
    if (fp_table && fp_table != stderr) fclose(fp_table);
    fp_table = NULL;
}
extern void trace(FILE* fp_trace, int level, const char *format, ...)
{
    va_list ap;
    ...
    if (!fp_trace||level>3) return;
    traceswap(fp_trace);
    /*fprintf(fp_trace,"%d ",level);*/
    va_start(ap,format); vfprintf(fp_trace,format,ap); va_end(ap);
    fflush(fp_trace);
}
extern void tracet(FILE* fp_trace, int level, const char *format, ...)
{
    va_list ap;
...
extern void tracemat(FILE* fp_trace, int level, const double *A, int n, int m, int p, int q)
{
    if (!fp_trace||level>3) return;
    matfprint(A,n,m,p,q,fp_trace); fflush(fp_trace);
}
extern void tracetable(FILE* fp_table, int level, const obsd_t* obs, int n) {
    char str[64], id[16];
    int i;

    if (!fp_table || level > 3) return;
    for (i = 0; i < n; i++) {
        time2str(obs[i].time, str, 3);
        satno2id(obs[i].sat, id);
        fprintf(fp_table, "table %s %-3s rcv%d %1d %1d %1d %1d %1d %1d\n",
            str, id, obs[i].rcv, (obs[i].L[0]==0)?0:1, (obs[i].L[1]==0)?0:1, (obs[i].P[0]==0)?0:1,
            (obs[i].P[1]==0)?0:1, (obs[i].SNR[0]==0)?0:1, (obs[i].SNR[1]==0)?0:1);
    }
    fflush(fp_table);
}
extern void traceobs(FILE* fp_trace, int level, const obsd_t *obs, int n)
{
    char str[64],id[16];
    int i;
...
extern void tracenav(FILE* fp_trace, int level, const nav_t *nav)
{
    ...
    for (i=0;i<nav->n;i++) {
        ...
        satno2id(nav->eph[i].sat,id);
        fprintf(fp_trace,"(%3d) %-3s : %s %s %3d %3d %20.14f %02x\n",i+1,
                id,s1,s2,nav->eph[i].iode,nav->eph[i].iodc, nav->eph[i].f0,nav->eph[i].svh);
    }
...
extern void tracegnav(FILE* fp_trace, int level, const nav_t *nav)
{
    char s1[64],s2[64],id[16];
    int i;
...
extern void tracehnav(FILE* fp_trace, int level, const nav_t *nav)
{
    char s1[64],s2[64],id[16];
    int i;
...
extern void tracepeph(FILE* fp_trace, int level, const nav_t *nav)
{
    char s[64],id[16];
    int i,j;
...
extern void tracepclk(FILE* fp_trace, int level, const nav_t *nav)
{
    char s[64],id[16];
    int i,j;
...
extern void traceb(FILE* fp_trace, int level, const uint8_t *p, int n)
{
    int i;
    if (!fp_trace||level>3) return;
                        ...
                        double humi)
{
    ...
    trpw=0.0022768*(1255.0/temp+0.05)*e/cos(z);
    return trph+trpw;
}
#ifndef IERS_MODEL
...
* compute tropospheric mapping function by
...
/* rms */
extern double rms(double* arr, int n) {
    int i; double sumsq=0;
    for (i = 0; i < n; i++) {
        sumsq += arr[i] * arr[i];
    }
    return SQRT(sumsq/n);
...
extern double means(double* arr, int n) {
    int i; double mean = 0;
    for (i = 0; i < n; i++) {
        mean += arr[i];
    }
...
extern double stds(double* arr, int n) {
    int i; double mean = 0, sumsq = 0;
    mean =means(arr,n);
    for (i = 0; i < n; i++) {
