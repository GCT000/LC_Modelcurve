...
extern int crc_D3search(rtcm_t* rtcm, FILE* fp) {
    if (rtk_crc24q(rtcm->buff, rtcm->len) != getbitu(rtcm->buff, rtcm->len * 8, 24)) {
        //可能是校验不通过 可能是丢包 向后寻找D3判断是不是百分百校验不通过 如果不能确定则产生新电文
        //退出当前进程会循环读取新数据 有新的D3应将数据前移 新D3电文若长度小于当前buff不操作 大于需退出读取新数据
        int i;
        for (i = 1; i < rtcm->len + 3; i++) {
            if (getbitu(rtcm->buff, i * 8, 8) == 211) {
                //printf("\n探测到新D3，该段电文可能存在丢包，更新电文\n");
                int j;
                for (j = 0; j < rtcm->len + 3; j++) {
                    rtcm->buff[j] = rtcm->buff[j + i];//数据前移
                }
                if (rtcm->nbyte == 0) rtcm->nbyte = rtcm->len + 3 - i;//更新nbyte
                else rtcm->nbyte = rtcm->nbyte + 3 - i;
                if (rtcm->nbyte < 3) {
                    /*for (int i = 0; i < rtcm->nbyte; i++) {
                        printf("%02X ", rtcm->buff[i]);
                    }*/
                    return 0;//不能读取电文长度 读取新数据
                }
                rtcm->len = getbitu(rtcm->buff, 14, 10) + 3;//更新len

                if (rtcm->len + 3 > rtcm->nbyte) {
                    /*for (int i = 0; i < rtcm->nbyte; i++) {
                        printf("%02X ", rtcm->buff[i]);
                    }*/
                    return 0;//新电文数据不够 读取新数据
                }
                else {
                    /*for (int i = 0; i < rtcm->len + 3; i++) {
                        printf("%02X ", rtcm->buff[i]);
                    }
                   printf("\n");*/
                    return crc_D3search(rtcm, fp);
                }
                return 0;
            }
        }
        //printf("\n该段电文未通过crc-24q校验\n");//没有return 0，说明不存在d3
        if (rtcm->len + 3 < rtcm->nbyte) {
            fseek(fp, rtcm->len + 3 - rtcm->nbyte, SEEK_CUR);
        }
        rtcm->nbyte = 0;
        return 0;
    }
        ...
        if (rtcm->len + 3 < rtcm->nbyte) {
            fseek(fp, rtcm->len + 3 - rtcm->nbyte, SEEK_CUR);//电文后仍有数据 文件指针前移 重新读取
            rtcm->nbyte = 0;
        }
        return 1;
    }
...
*            
*-----------------------------------------------------------------------------*/
extern int input_rtcm3(rtcm_t *rtcm, uint8_t data, FILE* fp)
{
    /* synchronize frame */
    if (rtcm->nbyte==0) {
    ...
    /* check parity */
    int crc_D3flag = crc_D3search(rtcm, fp);
    if (!crc_D3flag)
        return crc_D3flag;
    /* decode rtcm3 message */
    else
        return decode_rtcm3(rtcm, NULL, NULL);
}
...
extern int input_rtcm3f(rtcm_t *rtcm, FILE *fp)
{
    ...
    for (i=0;i<4096;i++) {
        if ((data=fgetc(fp))==EOF) return -2;
        if ((ret=input_rtcm3(rtcm,(uint8_t)data, fp))) return ret;
    }
    return 0; /* return at every 4k bytes */
}
