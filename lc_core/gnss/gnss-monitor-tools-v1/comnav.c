...
static int decode_rangecmpb(raw_t *raw)
{
    ...
    /*trace(5,"decode_rangecmpb: len=%d\n",raw->len);*/
    ...
    for (i=0,p+=4;i<nobs;i++,p+=24) {
        ...
        if (!(sat=satno(sys,prn))) {
            /*trace(5,"cnav rangecmpb satellite number error: sys=%d,prn=%d\n",sys,prn);*/
            continue;
        }
        ...
        if ((wavelen= CLIGHT / (freq == 0 ? FREQ1_GLO : FREQ2_GLO))<=0.0) {
            if (sys==SYS_GLO) wavelen=CLIGHT/(freq==0?FREQ1_GLO:FREQ2_GLO);
            else wavelen= CLIGHT / sat2freq(sat, code, &raw->nav);
        }
        adr=I4(p+12)/256.0;
        ...
        if ((index=obsindex(&raw->obs,raw->time,sat))>=0) {
            ...
            if (code==CODE_L2X) {
                ...
                /*trace(5,"cnav L2C phase shift corrected: prn=%2d\n",prn);*/
            }
#endif
        }
    }
...
static int decode_rangeb(raw_t *raw)
{
    ...
    for (i=0,p+=4;i<nobs;i++,p+=44) {
        ...
        if (!(sat=satno(sys,prn))) {
            /*trace(5,"cnav rangeb satellite number error: sys=%d,prn=%d\n",sys,prn);*/
            continue;
        }
        if (sys==SYS_GLO&&!parity) continue; /* invalid if GLO parity unknown */
...
static int decode_rawephemb(raw_t *raw)
{
    ...
    /*trace(5,"decode_rawephemb: len=%d\n",raw->len);*/
...
static int decode_rawwaasframeb(raw_t *raw)
{
    ...
    /*trace(5,"decode_rawwaasframeb: len=%d\n",raw->len);*/
...
{
    /*trace(5,"decode_rawsbasframeb: len=%d\n",raw->len);*/
...
static int decode_gloephemerisb(raw_t *raw)
{
    ...
    /*trace(5,"decode_gloephemerisb: len=%d\n",raw->len);*/
