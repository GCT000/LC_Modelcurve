                   ...
                   double *azel, int *vsat, double *resp, int *ns)
{
    ...
    for (i=*ns=0;i<n&&i<MAXOBS;i++) {
        ...
        /* reject duplicated(�ظ�) observation data */
        ...
        /*if unhealth, svh[i]>0, exclude this satellite*/
        ...
        /*trace(5,"sat=%2d azel=%5.1f %4.1f res=%7.3f sig=%5.3f\n",obs[i].sat,
    ...
    for (i=0;i<MAXITR;i++) {
        ...
        if (norm(dx,NX)<1E-4) {
            /*for (int index = 0; index < n; index++) {
                trace(2, "QUILITY CHECK %s\tsat=%d\tazel=\t%5.1f\t%4.1f\tres=\t%7.3f\tsig=\t%5.3f\tsnr=\t%5.2f\t%5.2f\n", tmptime,obs[index].sat,
                    azel[index * 2] * R2D, azel[1 + index * 2] * R2D, resp[index], sqrt(var[index]),obs[index].SNR[0]*SNR_UNIT, obs[index].SNR[1] * SNR_UNIT);
            }*/
            sol->type=0;
...
extern int estpos1(const obsd_t* obs, int n, const double* rs, const double* dts,
    const double* vare, const int* svh, const nav_t* nav,
    const prcopt_t* opt, const ssat_t* ssat, sol_t* sol, double* azel,
    int* vsat, double* resp, char* msg)
{
...
* return : status(1:ok,0:error)
*-----------------------------------------------------------------------------*/
extern int pntpos(FILE* fp_trace, const obsd_t *obs, int n, const nav_t *nav,
                  const prcopt_t *opt,rtk_t* rtk, sol_t *sol, double *azel, ssat_t *ssat,
                  char *msg)
{
    prcopt_t opt_=*opt;
    double *rs,*dts,*var,*azel_,*resp;
    int i, j,stat,vsat[MAXOBS]={0},svh[MAXOBS], sys0;
    ...
    if (rtk != NULL)
    {
        for (i = 0; i < 3; i++) {
            for (j = 0; j < NSYS + 1; j++) {
                rtk->dpos[obs->rcv - 1][j][i] = 0.0;
                rtk->dposFlag[obs->rcv - 1][j][i] = 0;
            }
        }
    }
    ...
    /*TDCP*/
    if (stat && rtk != NULL && (rtk->tt == 0.0 || rtk->tt - 20.0 < 1E-3))
    {
        for (j = 0; j < NSYS; j++)
        {
            if (j == 0) sys0 = SYS_GPS;
            ...
            rtk->dposFlag[obs->rcv - 1][j][0] = getTimeBaseLine(fp_trace, obs, n, sys0, nav, &opt_, rtk, sol, rs, dts, svh);
        }
    }

    if (rtk != NULL) matcpy(sol->prr, sol->rr, 3, 1);
    if (rtk != NULL)
    {
        getMultipath(obs, n, nav, &opt_, rtk, ssat);
    }
