...
static int readsp3h(FILE *fp, gtime_t *time, char *type, int *sats,
                    double *bfact, char *tsys,int *satsstd)
{
    ...
    while (fgets(buff,sizeof(buff),fp)) {
        ...
        if (buff[0]=='#'&&(buff[1]=='c'||buff[1]=='d')) {
            *type=buff[2];
            ...
            for (int g = 0; g < 17; g++)
            {
                satsstd[g + indexstd * 17] = (int)str2num(buff, 11 + 3 * g, 1);
            }
            indexstd++;
        }
        else if (i==2*nl+11){
...
static void readsp3b(FILE *fp, char type, int *sats, int ns, double *bfact,
                     char *tsys, int index, int opt, nav_t *nav,int *satsstd)
{
    peph_t peph;
    gtime_t time;
    double val,std,base;
    ...
    while (fgets(buff,sizeof(buff),fp)) {
        ...
        for (i=pred_o=pred_c=v=0;i<n&&fgets(buff,sizeof(buff),fp);i++) {
            ...
            int indexsat = 0;
            for (; indexsat < n; indexsat++)
            {
                if (prn == sats[indexsat])
                {
                    break;
                }
                else { continue; }
            ...
            for (j=0;j<4;j++) {
                ...
                double index_std = str2num(buff, 61 + j * 3, j < 3 ? 2 : 3);
                if (index_std != 0)
                {
                    std = index_std;
                }
                else { std = satsstd[indexsat]; }
...
extern void readsp3(const char *file, nav_t *nav, int opt)
{
    ...
    gtime_t time={0};
    double bfact[2]={0};
    int i,j,n,ns,sats[MAXSAT]={0},satsstd[MAXSAT] = { 0 };;
    ...
    for (i=j=0;i<n;i++) {
        ...
        /* read sp3 header */
        ns=readsp3h(fp,&time,&type,sats,bfact,tsys,satsstd);
        
        /* read sp3 body */
        readsp3b(fp,type,sats,ns,bfact,tsys,j++,opt,nav,satsstd);
