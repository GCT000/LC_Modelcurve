...
#define VER  "v1.1.4"             /* library version */
...
#define MAXEXFILE   50                /* max number of expanded files */
...
#define RANDOM_EL   1                   /* elevation random model */
#define RANDOM_SNR  2                   /* SNR random model */
#define RANDOM_COM  3                   /* randdom model combined elevation and SNR */
...
#define MAX_STA_NUM 10000   /* max num, not used now */
...
typedef struct {        /* solution type */
    ...
    double prr[6];      /* previous position/velocity (m|m/s) */
...
typedef struct {        /* processing options type */
    ...
    double prc_int;     /* processing interval(s) (0:all) */
    int format;         /* rtcm:1 obs:2*/
    int flt_len;
    int prc_itv;
...
typedef struct {        /* satellite status type */
    ...
    uint8_t fix [NFREQ]; /* ambiguity fix flag (1:float,2:fix,3:hold)  corrected by guo*/
    ...
    gtime_t tpt[2][NFREQ]; /* previous tdcp carrier-phase time */
    double  tph[2][NFREQ]; /* previous tdcp carrier-phase observable (cycle) */
    double  php[2][NFREQ];/* carrier-phase observable prealign (cycle) */
    double  prs[2][3];    /* previous satellite position (m) */
    double  pvs[2][3];    /* previous satellite velocity (m/s) */
    double  pts[2];       /* previous satellite clock (s) */
    uint8_t vsphase[2];   /* carrier-phase observable valid index (s) */
    double crtmp[NFREQ];
    int nmp[NFREQ];
    double mp[NFREQ];
    double sdmp[NFREQ];
    uint16_t arc_rover;
    uint16_t arc_base;
    double  pv[2][NFREQ]; /* previous pseudorange and carrier-phase residuals */
    int exp[NFREQ];       /* excluded pseudorange observations (0:off; 1:on) */
    int exc[NFREQ];       /* excluded carrier-phase observations (0:off; 1:on) */
    gtime_t exptime[NFREQ];
    gtime_t exctime[NFREQ];
    int mark;
} ssat_t;
...
typedef struct {        /* RTK control/result type */
    ...
    uint8_t dposFlag[2][NSYS + 1][3];
    double dpos[2][NSYS + 1][3];
    double qdpos[2][NSYS + 1][6];
    long int fixnum;
    double hasL[MAXSAT][NFREQ];
    double hasP[MAXSAT][NFREQ];
    int rerunIndex;              /* rerun on-off (0:off) */
    int rerun;                   /* index of rerun (0:off) */
} rtk_t;
...
typedef struct monitorInfo {
    char configStr[600];    /* command string invoked afterwards */
    char errMsg[1000];
    char solBuf[400];
    double blh[3];       /* rover position b l h */
    double enu[3];
}mInfo;

typedef struct {         /* monitor type */
    pcvs_t pcvss;        /* receiver antenna parameters */
    pcvs_t pcvsr;        /* satellite antenna parameters */
    obs_t obss;          /* observation data */
    nav_t navs;          /* navigation data */
    sbs_t sbss;          /* sbas messages */
    sta_t stas[MAXRCV];  /* station infomation */
    int nepoch;          /* number of observation epochs */
    int nitm;            /* number of invalid time marks */
    ...
    int iitm;            /* current invalid time mark index */
    int revs;            /* analysis direction (0:forward,1:backward) */
    ...
    int isolf;           /* current forward solutions index */
    int isolb;           /* current backward solutions index */
    ...
    gtime_t invalidtm[100];/* invalid time marks */
    rtcm_t rtcm;             /* rtcm control struct */
    FILE* fp_rtcm;       /* rtcm data file pointer */
    int nepoch_u, nepoch_r;
    gtime_t t_u, t_r;
    gtime_t t_p;         /* previous process time*/
    int nnav_u, nnav_r;
    int obsflag;         /* 1: rover no new data 2:base no new data 4: rover insuffcient 8: base insuffcient*/
    int navflag;         /* 1: rover no nav 2:base no nav 4: nav insuffcient */
    int validFlag;       /* valid flag 0:invalid 1:valid */
    int fixNum;          /* fixed epochs */
    int fixOrFloatNum;
    int epochNum;
    double blh[3];       /* rover position b l h */
    double enu[3];       /* base length vector */
    double bLength;      /* base length */
    double nSatUsed;     /* average used satellites */
    gtime_t ts, te;       /* start time and end time */
    double* dpos;
    int ti;
    int ti_r;
    int ti_u;
...
EXPORT void traceopen();
EXPORT void traceclose(FILE* fp_trace);
EXPORT void tableclose(FILE* fp_table);
EXPORT void trace    (FILE* fp_trace, int level, const char *format, ...);
EXPORT void tracet   (FILE* fp_trace, int level, const char *format, ...);
EXPORT void tracemat (FILE* fp_trace, int level, const double *A, int n, int m, int p, int q);
EXPORT void tracetable(FILE* fp_table, int level, const obsd_t* obs, int n);
EXPORT void traceobs (FILE* fp_trace, int level, const obsd_t *obs, int n);
EXPORT void tableopen();
EXPORT void tracenav (FILE* fp_trace, int level, const nav_t *nav);
EXPORT void tracegnav(FILE* fp_trace, int level, const nav_t *nav);
EXPORT void tracehnav(FILE* fp_trace, int level, const nav_t *nav);
EXPORT void tracepeph(FILE* fp_trace, int level, const nav_t *nav);
EXPORT void tracepclk(FILE* fp_trace, int level, const nav_t *nav);
EXPORT void traceb   (FILE* fp_trace, int level, const uint8_t *p, int n);
...
EXPORT int input_rtcm3 (rtcm_t *rtcm, uint8_t data, FILE* ofp);
...
EXPORT void outsol  (FILE *fp, const sol_t *sol, const double *rb,
                     const solopt_t *opt,gtime_t *outtime);
...
EXPORT int pntpos(FILE* fp_trace, const obsd_t *obs, int n, const nav_t *nav,
                  const prcopt_t *opt,rtk_t* rtk, sol_t *sol, double *azel,
...
EXPORT int estpos1(const obsd_t* obs, int n, const double* rs, const double* dts,
    const double* vare, const int* svh, const nav_t* nav,
    const prcopt_t* opt, const ssat_t* ssat, sol_t* sol, double* azel,
    int* vsat, double* resp, char* msg);
...
EXPORT int rtkpos(FILE* fp_trace, FILE* fp_table, rtk_t* rtk, const obsd_t* obs, int nobs, const nav_t* nav);
...
/* tdcp functions ------------------------------------------------------------*/
EXPORT int tdcpres(const prcopt_t* opt, const nav_t* navs, obsd_t* pre_obs, obsd_t* last_obs, double* pre_dts, double* last_dts,
    double* pre_e, double* last_e, double* pre_rs, double* last_rs, sol_t* pre_sol,
    sol_t* last_sol, double* H, double* L, int n, double* x);
EXPORT int tdcprocess(rtk_t* rtk, obsd_t* pre_obs, int pren, obsd_t* last_obs, int lastn, nav_t* navs, ssat_t* pre_ssat, FILE* speedfile);
EXPORT void tdcp(rtk_t* rtk, obsd_t* pre_obs, int pren, obsd_t* last_obs, int lastn, nav_t* navs);
EXPORT int searchobs(rtk_t* rtk, obsd_t* pre_obs, int pren, obsd_t* last_obs, int lastn, int nf);
EXPORT int ols(double* H, double* P, double* L, double* x, int m, int n);
EXPORT void calspeed(double* deltax, int n, double sec, double* vel);
EXPORT void covbyelev(prcopt_t* opt, double* P, double* pre_azel, double* last_azel, int n);
...
EXPORT int getTimeBaseLine(FILE *fp_trace, const obsd_t* obs, int n, int sys0, const nav_t* nav, prcopt_t* opt, rtk_t* rtk, sol_t* sol, double* rs, double* dts, int* svh);
EXPORT int getRefTimeBaseLine(const obsd_t* obs, int n, const nav_t* nav, const prcopt_t* opt, rtk_t* rtk, sol_t* sol);
EXPORT void getMultipath(const obsd_t* obs, int n, const nav_t* nav, const prcopt_t* opt, rtk_t* rtk, ssat_t* ssat);

EXPORT void sort_v(double* v, int* isat, int len);
EXPORT double rms(double* arr, int n);
EXPORT double stds(double* arr, int n);
EXPORT double means(double* arr, int n);

EXPORT int outsolMonitor(uint8_t* buff, const sol_t* sol, const double* rb,
    const solopt_t* opt);
EXPORT sol_t sortSol(sol_t* solArray, int n, const double* rb);
EXPORT int outnmea_gqdjs(uint8_t *buff,monitor_t *monitor);
...
/* variables for license function --------------------------------------------*/
// lock_t lic_lock;
// map_t sta_num[MAX_STA_NUM];     // shared variable
static time_t timep;
