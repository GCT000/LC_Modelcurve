...
/* output solution in the form of NMEA GQDJS sentence --------------------------*/
extern int outnmea_gqdjs(uint8_t *buff,monitor_t *monitor)
{
    gtime_t time;
    char tsStr[20],teStr[20];
    double ep[6],dms1[3],dms2[3];
    double fixRatio=((double)monitor->fixNum)/monitor->nepoch;
    char *p=(char *)buff,*q,sum;

    time2epoch(monitor->ts,ep);
    sprintf(tsStr,"%4d%02d%02d%02d%02d%02d",(int)ep[0],(int)ep[1],(int)ep[2],(int)ep[3],(int)ep[4],(int)ep[5]);
    time2epoch(monitor->te,ep);
    sprintf(teStr,"%4d%02d%02d%02d%02d%02d",(int)ep[0],(int)ep[1],(int)ep[2],(int)ep[3],(int)ep[4],(int)ep[5]);

    if (monitor->validFlag<=0) {
        p+=sprintf(p,"$%s,%s,%s,,,,,,,,,,,,","GQDJS",tsStr,teStr);
        for (q=(char *)buff+1,sum=0; *q; q++) sum^=*q;
        p+=sprintf(p,"*%02X%c%c",sum,0x0D,0x0A);
        return p-(char *)buff;
    }

    deg2dms(fabs(monitor->blh[0])*R2D,dms1,7);
    deg2dms(fabs(monitor->blh[1])*R2D,dms2,7);
    p+=sprintf(p,"$%s,%s,%s,%d,"
               "%02.0f%010.7f,%s,%03.0f%010.7f,%s,"
               "%.4f,%.3f,%.1f,%.4f,%.4f,%.4f,%.4f",
               "GQDJS",tsStr,teStr,monitor->validFlag,
               dms1[0],dms1[1]+dms1[2]/60.0,monitor->blh[0]>=0 ? "N" : "S",dms2[0],dms2[1]+dms2[2]/60.0,monitor->blh[1]>=0 ? "E" : "W",
               monitor->blh[2],fixRatio,monitor->nSatUsed,monitor->enu[0],monitor->enu[1],monitor->enu[2],norm(monitor->enu,3));
    for (q=(char *)buff+1,sum=0; *q; q++) sum^=*q; /* check-sum */
    p+=sprintf(p,"*%02X\r\n",sum);
    return p-(char *)buff;
}
...
extern int outprcopts(uint8_t *buff, const prcopt_t *opt)
{
    ...
    p+=sprintf(p,"%s calculate mode  : %s\r\n",COMMENTH,s1[opt->mode]);
...
extern sol_t sortSol(sol_t* solArray, int n, const double* rb) {
    sol_t tempSol = { 0 };
    if (n <= 0) return tempSol;
    int i, j, res;
    double* lineArray;
    double pos[3], rr[3], enu[3], P[9], Q[9];
    double temp; 
    lineArray = (double*)malloc(sizeof(double) * n);
    for (i = 0; i < n; i++) {
        for (j = 0; j < 3; j++) rr[j] = solArray[i].rr[j] - rb[j];
        ecef2pos(rb, pos);
        soltocov(&solArray[i], P);
        covenu(pos, P, Q);
        ecef2enu(pos, rr, enu);
        lineArray[i] = norm(enu, 3);
    }
    for (i = 0; i < n-1; i++)
    {
        for (j = i + 1; j < n; j++)
        {
            if (lineArray[j] < lineArray[i])
            {
                temp = lineArray[i];
                tempSol = solArray[i];
                lineArray[i] = lineArray[j];
                solArray[i] = solArray[j];
                lineArray[j] = temp;
                solArray[j] = tempSol;
            }
        }
    }
    tempSol = solArray[n / 2];
    free(lineArray);
    return tempSol;
}

extern int outsolMonitor(uint8_t* buff, const sol_t* sol, const double* rb,
    const solopt_t* opt)
{
    gtime_t time, ts = { 0 };
    double gpst;
    ...
    covenu(pos, P, Q);
    ecef2enu(pos, rr, enu);
    if (opt->posf == SOLF_XYZ) matcpy(enu, sol->rr, 1, 3);
    p += sprintf(p, " %.4f %.4f %.4f %s",
        enu[0], enu[1], enu[2], sol->stat == 1 ? "Fixed" : "Float");
    if (opt->posf != SOLF_XYZ&&(fabs(enu[0] > 99999.999) || fabs(enu[1] > 99999.999) || fabs(enu[2] > 99999.999)))
    {
        printf("f(outsolMonitor), abnormal value detected!!\n,enu[0]=%lf enu[1]=%lf enu[2]=%lf", enu[0], enu[1], enu[2]);
    }
    return (int)(p - buff);
}
