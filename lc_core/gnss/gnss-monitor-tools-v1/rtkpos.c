...
*           2009/09/03 1.7  add check of valid satellite 
...
static void init_ex(int* exvalue) {
    if (exvalue) {
        int j;
        for (j = 0; j < NFREQ; j++) {
            exvalue[j] = 0;
                     ...
                     double bl, double dt, int f, const prcopt_t *opt, const obsd_t *obs)
{
    ...
    double fact=1.0;
    double sinel=sin(el),var = 0;
    double c_Li = 0;
    int s_base = 0, s_rover = 0;
    ...
    if (opt->rdmod == RANDOM_EL || opt->rdmod == RANDOM_COM) {
        /* adjust variance for config parameters */
        a = fact * opt->err[1];  /* base term */  /* {0.0,0.003,0.003,0.0,1.0,52.0,0.0,0.0} */
    ...
    else if (opt->rdmod == RANDOM_SNR) {
        s_base = (int)((snr_base * 0.1) / 5) > 9 ? 9 : (int)((snr_base * 0.1) / 5);
        s_rover = (int)((snr_rover * 0.1) / 5) > 9 ? 9 : (int)((snr_rover * 0.1) / 5);
        if (frq == 0) {
            c_Li = CL1;
        }
        else if (frq == 1) {
        ...
        var = fact * fact * c_Li * (log10(-s_base / 2.0) + log10(-s_rover / 2.0));
    }
    if (opt->rdmod == RANDOM_COM && opt->err[6]>0) {  /* add SNR term */
        e=fact*opt->err[6];
...
static int selsat(rtk_t* rtk, obsd_t *obs, double *azel, int nu, int nr,
                  const prcopt_t *opt, int *sat, int *iu, int *ir)
{
    ...
    char tmptime[128];
    time2str(obs[0].time, tmptime, 2);
    
    for (i=0,j=nu;i<nu&&j<nu+nr;i++,j++) {
        if      (obs[i].sat<obs[j].sat) j--;
        else if (obs[i].sat>obs[j].sat) i--;
        else if (azel[1+j*2]>=opt->elmin) { /* elevation at base station */
            if ((rtk->ssat[obs[i].sat - 1].exc[0]==1&&rtk->ssat[obs[i].sat-1].exc[1]==1)||(rtk->ssat[obs[j].sat-1].exc[0]==1&&rtk->ssat[obs[j].sat-1].exc[1]==1)) continue;  
            /*�����в����󣬽�exSat���Ϊ1������Ӧ���ز���α��۲�ֵ��Ϊ0���Ա����´���ʱ�ų������ǵ�Ӱ��*/
            int frq;
            for (frq = 0; frq < NFREQ; frq++) {
                if (timediff(rtk->sol.time, rtk->ssat[obs[i].sat - 1].exctime[frq]) < 10) obs[i].L[frq] = 0;   /*�����������ǣ��ڱ����Ԫ֮��10���ڲ���ʹ��*/
                /*if (timediff(rtk->sol.time, rtk->ssat[obs[i].sat - 1].exptime[frq]) < 10) obs[i].P[frq] = 0;*/
                if (timediff(rtk->sol.time, rtk->ssat[obs[j].sat - 1].exctime[frq]) < 10) obs[j].L[frq] = 0;
                /*if (timediff(rtk->sol.time,rtk->ssat[obs[j].sat - 1].exptime[frq])<10) obs[j].P[frq] = 0;*/
            }
            sat[k]=obs[i].sat; iu[k]=i; ir[k++]=j;
            /*trace(5, "(%2d) sat=%3d iu=%2d ir=%2d\n", k - 1, obs[i].sat, i, j);*/
        }
    }
    return k;
}
...
static void udpos(rtk_t *rtk, double tt)
{
    ...
    dpos=SQRT(SQR(rtk->dpos[0][0][0])+SQR(rtk->dpos[0][0][1])+SQR(rtk->dpos[0][0][2]));
    
    if (rtk->dposFlag[0]==1&&dpos<0.1) {
        for (i = 0; i < 3; i++) {
            rtk->P[i+i*rtk->nx]+=0.00001;
        }
        return;
    }
    if (!rtk->opt.dynamics) {
                           ...
                           const nav_t *nav)
{
    ...
    for (k=1;k<rtk->opt.nf;k++) {
        ...
        if (gf0!=0.0&&fabs(gf1-gf0)>rtk->opt.thresslip) {
            ...
            /*printf("%s slip detected GF jump (sat=%2d L1-L%d dGF=%.3f)\n",
                tmptime,sat,k+1,gf0-gf1);*/
        }
    }
}
...
static double mwmeas(const obsd_t* obs,int i,int j,int k, const nav_t* nav)
{
    ...
    P2 = sdobs(obs, i, j, k + NFREQ);
    if (freq1 == 0.0 || freq2 == 0.0 || L1 == 0.0 || L2 == 0.0 || P1 == 0.0 || P2 == 0.0) return 0.0;
    return (L1 - L2) * CLIGHT / (freq1 - freq2) - (freq1 * P1 + freq2 * P2) / (freq1 + freq2);
}

/* detect slip by Melbourne-Wubbena linear combination jump ------------------*/
static void detslp_mw(rtk_t* rtk, const obsd_t* obs, int i, int j, const nav_t* nav)
{
    int k, sat = obs[i].sat;
    double w0, w1;
    /* skip check if slip already detected */
    ...
    for (k = 1; k < rtk->opt.nf; k++) {
        /* calc SD geomotry free LC of phase between freq0 and freqk */
        if ((w1 = mwmeas(obs, i, j, k, nav)) == 0.0) continue;

        w0 = rtk->ssat[obs[i].sat - 1].mw[0];
        rtk->ssat[obs[i].sat - 1].mw[0] = w1;

        if (w0 != 0.0 && fabs(w1 - w0) > THRESH_MW_JUMP) {
                   ...
                   const int *iu, const int *ir, int ns, const nav_t *nav)
{
    ...
    for (i=0;i<ns;i++) {
        ...
        /* detect cycle slip by LLI */
        for (k=0;k<rtk->opt.nf;k++) rtk->ssat[sat[i]-1].slip[k]&=0xFC;/*11111100,��֤slip�����������λΪ0*/
    ...
    for (k=0;k<nf;k++) {
        /* reset phase-bias if instantaneous AR or expire obs outage counter ��obs�жϼ���������) */
        for (i=1;i<=MAXSAT;i++) {
        ...
        for (i=0;i<ns;i++) {
            ...
            rtk->P[j+j*rtk->nx]+=rtk->opt.prn[0]*rtk->opt.prn[0]*fabs(tt);/*bias - noise std = 30m*/
                      ...
                      const prcopt_t *opt, double *y, double *freq)
{
    ...
    if (opt->ionoopt==IONOOPT_IFLC) { /* iono-free linear combination */
        ...
        for (i=0;i<nf;i++) {
            ...
            /*trace(5, "%s,%d,sat=%02d,L[%d]=%16.3f,lamda=%16.4f,P[%d]=%16.3f,%16.4f,%16.4f\n", tmptime, base, obs->sat, i + 1, obs->L[i], CLIGHT / freq[i], i + 1, obs->P[i], y[i], y[i+nf]);*/
        }
    }
}
...
static int zdres(FILE* fp_trace, int base, const obsd_t *obs, int n, const double *rs,
                  ...
                  int nv, double *R)
{
    ...
    for (b=0;b<n;k+=nb[b++]) {  /* loop through each system */
        ...
        for (i=0;i<nb[b];i++) for (j=0;j<nb[b];j++) {
            ...
            /*R[k + i + (k + j) * nv] = i==j ? Ri[k + i] + Rj[k + i] : 0.0;*/
        }
    }
}
...
static int ddres(FILE* fp_trace,rtk_t *rtk, const nav_t *nav, const obsd_t *obs, double dt, const double *x,
    ...
    for (m=0;m<6;m++) { 
        ...
        for (f=opt->mode>PMODE_DGPS?0:nf;f<nf*2;f++) {
            ...
            for (j=0;j<ns;j++) {
                ...
                /*trace(2, "ddres %s\t%14.8f=(%14.8f-%14.8f)-(%14.8f-%14.8f)\n", tmptime, v[nv], y[f + iu[i] * nf * 2], y[f + ir[i] * nf * 2], y[f + iu[j] * nf * 2], y[f + ir[j] * nf * 2]);*/
                ...
                if (!code) {
                    ...
                    if (opt->ionoopt!=IONOOPT_IFLC) {
                        ...
                        /*trace(2, "ddres %s\t%14.8f-=%14.8f*%14.8f-%14.8f*%14.8f\n", tmptime, v[nv], CLIGHT / freqi, x[IB(sat[i], frq, opt)], CLIGHT / freqj, x[IB(sat[j], frq, opt)]);*/
                        if (H) {
                ...
                char idi[32], idj[32];
                satno2id(sat[i], idi); satno2id(sat[j], idj);
                /*trace(5, "ddres %s\tsat=%s-%s %s%d v=%13.3f R=%9.6f %9.6f icb=%9.3f lock=%5d x=%9.3f\n", tmptime, idi,
                        idj, code ? "P" : "L", frq + 1, v[nv], Ri[nv], Rj[nv], icb,
...
static double intpres(FILE* fp_trace, gtime_t time, const obsd_t *obs, int n, const nav_t *nav,
                      rtk_t *rtk, double *y)
{
    obsd_t obsb[MAXOBS];
    double yb[MAXOBS*NFREQ*2],rs[MAXOBS*6],dts[MAXOBS*2],var[MAXOBS];
    double e[MAXOBS*3],azel[MAXOBS*2],freq[MAXOBS*NFREQ];
    int nb=0,svh[MAXOBS*2];
    prcopt_t *opt=&rtk->opt;
    ...
    if (!zdres(fp_trace, 1,obsb,nb,rs,dts,var,svh,nav,rtk->rb,opt,1,yb,e,azel,freq)) {
        return tt;
    }
    for (i=0;i<n;i++) {
...
static int ddidx(FILE* fp_trace, rtk_t *rtk, int *ix, int gps, int glo, int sbs,int* fix,int* freq)
{
    int i,j,k,m,f,n,nb=0,na=rtk->na,nf=NF(&rtk->opt),nofix;
    double ref[MAXSAT],fix_[MAXSAT];
    char tmptime[128];
    time2str(rtk->sol.time, tmptime, 2);
    /*trace(5,"ddmat: gps=%d/%d glo=%d/%d sbs=%d\n",gps,rtk->opt.gpsmodear,glo,rtk->opt.glomodear,sbs);*/
    ...
    for (m=0;m<6;m++) { /* m=0:GPS/SBS,1:GLO,2:GAL,3:BDS,4:QZS,5:IRN */
        ...
        for (f=0,k=na;f<nf;f++,k+=MAXSAT) {
            ...
            for (n=0,j=k;j<k+MAXSAT;j++) {
                ...
                /*trace(2, "LOCKINDEX:%s\t%d\t%d\n",tmptime, j-k+1, rtk->ssat[j - k].lock[f]);*/
    ...
    if (nb>0) {
        char tmptime[128];
        time2str(rtk->sol.time, tmptime, 2);
        // trace(fp_trace, 2, "ddres time =\t%s\tnb =\t%d\n", tmptime, nb);
                        
        trace(fp_trace, 2,"refSats=\n");tracemat(fp_trace, 2,ref,1,nb,10,0);
        trace(fp_trace, 2,"fixSats=\n");tracemat(fp_trace, 2,fix_,1,nb,10,0);
    }
    return nb;
}
...
{
    double* v, * H, * R,*mark_;
    int i,j,n,m,f,info,index[MAXSAT],mark[MAXSAT],nb=rtk->nx-rtk->na,nv=0,nf=NF(&rtk->opt);
    double dd,sum;
    char tmptime[128];
    time2str(rtk->sol.time, tmptime, 2);
    ...
    v = mat(nb, 1); H = zeros(nb, rtk->nx); mark_ = zeros(nb, 1);
    
    for (m=0;m<5;m++) for (f=0;f<nf;f++) {
        
        for (n=i=0;i<MAXSAT;i++) {
            /*if (rtk->ssat[i].mark == 1) trace(2, "sat %d var large\n", i + 1);*/
            if (!test_sys(rtk->ssat[i].sys,m)||rtk->ssat[i].fix[f]!=2||
                rtk->ssat[i].azel[1]<rtk->opt.elmaskhold) {
                continue;
            }
            mark[n] = 0;
            if (rtk->ssat[i].mark==1) mark[n] = 1;
            index[n++]=IB(i+1,f,&rtk->opt);
    ...
    if (rtk->opt.modear==ARMODE_FIXHOLD&&nv<rtk->opt.minholdsats) { 
        ...
        free(v); free(H); free(mark_);
        return;
    }
    
    rtk->holdamb=1;  /* set flag to indicate hold has occurred */
    R=zeros(nv,nv);
    double dpos = SQRT(SQR(rtk->dpos[0][0][0]) + SQR(rtk->dpos[0][0][1]) + SQR(rtk->dpos[0][0][2]));
    for (i = 0; i < nv; i++) {
        R[i + i * nv] = rtk->opt.varholdamb;
        if (rtk->dposFlag[0]==1&&dpos<0.1&&mark_[i] == 1) 
            R[i + i * nv] = 1;
    }
    ...
    free(R); free(v); free(H); free(mark_);
...
static int resamb_LAMBDA(FILE* fp_trace, rtk_t *rtk, double *bias, double *xa,int gps,int glo,int sbs)
{
    prcopt_t *opt=&rtk->opt;
    int i,j,nb,nb1,info,nx=rtk->nx,na=rtk->na;
    double *DP,*y,*b,*db,*Qb,*Qab,*QQ,*sigmaN,s[2];
    int *ix, fix[MAXSAT], freq[MAXSAT];
    ...
    while (1)
    {
        ...
        ix = imat(nx, 2);
        if ((nb = ddidx(fp_trace, rtk, ix, gps, glo, sbs, fix, freq)) < (rtk->opt.minfixsats - 1)) {  /* nb is sat pairs */
            errmsg(rtk, "not enough valid double-differences\n");
            free(ix);
            return -1; /* flag abort */
        }
        /*for (i = 0; i < nb; i++) {
            printf("%d\t%d\t%.4f\t%.4f\t%.8f\t%.8f\n", ix[2 * i], ix[2 * i + 1], rtk->x[ix[2 * i]], rtk->x[ix[2 * i + 1]],rtk->P[ix[2*i]+ix[2*i]*nx], rtk->P[ix[2 * i + 1] + ix[2 * i + 1] * nx]);
        }*/
        rtk->nb_ar = nb;
        ...
        Qab = mat(na, nb); QQ = mat(na, nb); sigmaN = zeros(1, nb);
        ...
        for (i = 0; i < nb; i++) {
            ...
            trace(NULL, 5, "y=%10.4f,x1=%10.4f,x2=%10.4f\n", y[i], rtk->x[ix[i * 2]], rtk->x[ix[i * 2 + 1]]);
        }
        /*trace(NULL, 5, "y=\n"); tracemat(fp_trace, 5, y, 1, nb, 14, 4);*/
        ...
        for (j = 0; j < nb; j++) for (i = 0; i < nb; i++) {
            ...
            if (i == j) sigmaN[j] = Qb[i + j * nb];
        }
        /*trace(NULL, 5, "Qb=\n"); tracemat(fp_trace, 5, Qb, nb, nb, 14, 4);*/
        for (j = 0; j < nb; j++) for (i = 0; i < na; i++) {
            Qab[i + j * na] = rtk->P[i + ix[j * 2] * nx] - rtk->P[i + ix[j * 2 + 1] * nx];
        }
        /*trace(NULL, 5, "Qab=\n"); tracemat(fp_trace, 5, Qab, na, nb, 14, 4);*/
        for (i = 0; i < nb; i++) QQb[i] = 1000 * Qb[i + i * nb];

        /*trace(2, "N(0)=\n"); tracemat(2, y, 1, nb, 10, 2);*/
        double rmsN = rms(sigmaN, nb);
        double stdN = stds(sigmaN, nb);

        trace(NULL, 2, "sigmaN =  "); tracemat(NULL, 2, sigmaN, 1, nb, 10, 7);
        trace(NULL, 2, "Qb*1000=  "); tracemat(NULL, 2, QQb, 1, nb, 10, 7);

        /* lambda/mlambda integer least-square estimation */
        /* return best integer solutions */
        /* b are best integer solutions, s are residuals */
        /*trace(2, "Qb    =   "); tracemat(2, Qb, nb, nb, 10, 7);*/
        trace(NULL, 2, "N(0)=\n"); tracemat(NULL, 2, y, 1, nb, 10, 2);
        if (!(info = lambda(nb, 2, y, Qb, b, s))) {
            trace(fp_trace, 2, "N(1)=     "); tracemat(fp_trace, 2, b, 1, nb, 7, 2);
            trace(NULL, 2, "N(2)=     "); tracemat(NULL, 2, b + nb, 1, nb, 7, 2);

            rtk->sol.ratio = s[0] > 0 ? (float)(s[1] / s[0]) : 0.0f;
            if (rtk->sol.ratio > 0 && rtk->sol.ratio < 3.0) {
                double std = stds(sigmaN, nb);
                double mean = means(sigmaN, nb);
                int rerun = 0;
                for (i = 0; i < nb; i++) {
                    if (sigmaN[i]<mean - 3 * std || sigmaN[i]>mean + 3 * std) {
                        rerun = 1;
                        /*trace(2, "out of threshold\t%s\t%2d\t%.8f\n", tmptime, fix[i], sigmaN[i]);*/
                        int prn = fix[i];
                        rtk->ssat[prn - 1].lock[freq[i]] = -15;
                        initx(rtk, 0.0, 0.0, IB(fix[i], freq[i], &rtk->opt));
                    }
                }
                if (rerun == 1) {
                    free(ix);
                    free(y); free(DP); free(b); free(db); free(Qb); free(Qab); free(QQ); free(sigmaN);
                    continue;
                }
            }
...
static int manage_amb_LAMBDA(FILE* fp_trace, rtk_t *rtk, double *bias, double *xa, const int *sat, int nf, int ns) 
{
    ...
    nb=resamb_LAMBDA(fp_trace, rtk,bias,xa,gps1,glo1,sbas1);
    ...
    if ((rtk->opt.navsys&SYS_GLO) && rtk->opt.glomodear==GLO_ARMODE_FIXHOLD && rtk->sol.ratio<rtk->sol.thres) {
        ...
        if (glo1!=glo2||gps1!=gps2)
            nb=resamb_LAMBDA(fp_trace, rtk,bias,xa,gps2,glo2,sbas2);
    }
                  ...
                  int nv, double thres)
{
    ...
    for (i=0;i<nv;i++) {
        ...
        if (v[i] * v[i] <= fact * R[i + i * nv]||type!=0) continue;
        if (rtk->rerunIndex) {
            switch (type)
            {
            case 0:
                rtk->ssat[sat2 - 1].exc[freq] = 1;
                rtk->ssat[sat2 - 1].exctime[freq] = rtk->sol.time;
                break;
            default:
                break;
            }
        ...
        errmsg(rtk,"large residual (sat=%2d-%2d %s%d v=%6.3f threshold=%6.3f sig=%.3f)\n",
              sat1,sat2,stype,freq+1,v[i],fact*R[i+i*nv],SQRT(R[i+i*nv]));
    }
    return stat;
}

extern void sort_v(double* v,int* isat,int len) {
    int i, j, isattemp;
    double vtemp;
    for (i = 0; i < len - 1; i++)
        for (j = 0; j < len - 1 - i; j++)
            if (fabs(v[j]) > fabs(v[j + 1])) {
                vtemp = v[j]; isattemp = isat[j];
                v[j] = v[j + 1]; isat[j] = isat[j + 1];
                v[j + 1] = vtemp; isat[j + 1] = isattemp;
            }
}
 ...
 */
static int relpos(FILE* fp_trace, rtk_t* rtk, const obsd_t* obs, int nu, int nr, const nav_t* nav)
{
    prcopt_t *opt=&rtk->opt;
    ...
    if (!zdres(fp_trace, 1,obs+nu,nr,rs+nu*6,dts+nu*2,var+nu,svh+nu,nav,rtk->rb,opt,1,
    ...
    if (opt->intpref) {
        dt=intpres(fp_trace, time,obs+nu,nr,nav,rtk,y+nu*nf*2);
    }
    ...
    trace(fp_trace,2,"TROP-EST : %14.4f, %14.4f\n",rtk->x[IT(0,&rtk->opt)],rtk->x[IT(1,&rtk->opt)]);
    ...
    for (i = 0; i < niter; i++) {
        /* calculate zero diff residuals [range - measured pseudorange] for rover (phase and code)
            output is in y[0:nu-1], only shared input with base is nav
                ...
                azel = [az, el] to sats                                   */
        /*trace(3, "rover:\n");*/
        if (!zdres(fp_trace, 0, obs, nu, rs, dts, var, svh, nav, xp, opt, 0, y, e, azel, freq)) {
            errmsg(rtk, "rover initial position error\n");
            stat = SOLQ_NONE;
            break;
        }
        trace(NULL, 5, "before filter y=\n"); tracemat(fp_trace, 5, y, nf * 2, n, 14, 4);
        ...
        if ((nv = ddres(NULL,rtk, nav, obs, dt, xp, Pp, sat, y, e, azel, freq, iu, ir, ns, v, H, R, vflg)) < 1) {
            errmsg(rtk, "no double-differenced residual\n");
            stat = SOLQ_NONE;
            break;
        }
        trace(NULL, 5, "before filter v=\n"); tracemat(fp_trace, 5, v, 1, ny, 14, 4);
    ...
    if (stat != SOLQ_NONE && zdres(fp_trace, 0, obs, nu, rs, dts, var, svh, nav, xp, opt, 0, y, e, azel,
        freq)) {
        trace(NULL, 5, "after filter y=\n"); tracemat(fp_trace, 5, y, nf * 2, n, 14, 4);
        /* calc double diff residuals again after kalman filter update for float solution */
        nv = ddres(fp_trace, rtk, nav, obs, dt, xp, Pp, sat, y, e, azel, freq, iu, ir, ns, v, NULL, R, vflg);
        trace(NULL, 5, "after filter v=\n"); tracemat(fp_trace, 5, v, 1, ny, 14, 4);
        
        for (i = 0; i < MAXSAT; i++) {
            init_ex(rtk->ssat[i].exc);
            init_ex(rtk->ssat[i].exp);
        }
        rtk->rerunIndex = 1;
        ...
        while (rtk->rerun) {
            ...
            if ((ns = selsat(rtk, obs, azel, nu, nr, opt, sat, iu, ir)) <= 0) {
                ...
                free(rs); free(dts); free(var); free(y); free(e); free(azel);
                free(freq); free(xp); free(Pp); free(xa); 
                return 0;
            }
            for (i = 0; i < MAXSAT; i++) {
                init_ex(rtk->ssat[i].exc);
            ...
            if (stat != SOLQ_NONE && zdres(fp_trace, 0, obs, nu, rs, dts, var, svh, nav, xp, opt, 0, y, e, azel,
                freq)) {
                /* calc double diff residuals again after kalman filter update for float solution */
                nv = ddres(NULL, rtk, nav, obs, dt, xp, Pp, sat, y, e, azel, freq, iu, ir, ns, v, NULL, R, vflg);
            }
            rtk->rerun = 0;  /*��rerun��־����*/
            rtk->rerunIndex = 1;
            valpos(rtk, v, R, vflg, nv, 3.0);
            rtk->rerunIndex = 0;
            if (niter > 10) {
                free(rs); free(dts); free(var); free(y); free(e); free(azel);
                free(freq);
    ...
    if (stat != SOLQ_NONE) {
        ...
        if (manage_amb_LAMBDA(fp_trace, rtk, bias, xa, sat, nf, ns) > 1) {
            trace(NULL, 5, "xa=\n"); tracemat(fp_trace, 5, xa, rtk->nx, 1, 16, 4);
            /* find zero-diff residuals for fixed solution */
            if (zdres(fp_trace, 0, obs, nu, rs, dts, var, svh, nav, xa, opt, 0, y, e, azel, freq)) {

                /* post-fit residuals for fixed solution (xa includes fixed phase biases, rtk->xa does not) */
                nv = ddres(NULL, rtk, nav, obs, dt, xa, NULL, sat, y, e, azel, freq, iu, ir, ns, v, NULL, R,
                    vflg);
...
extern void rtkinit(rtk_t *rtk, const prcopt_t *opt)
{
    ...
    for (i = 0; i < 2; i++) {
        for (j = 0; j < NSYS + 1; j++) {
            for (k = 0; k < 3; k++) rtk->dpos[i][j][k] = 0;
            for (k = 0; k < 6; k++) rtk->qdpos[i][j][k] = 0;
        }
    }
    rtk->rerun = 0;
    rtk->rerunIndex = 0;
...
*-----------------------------------------------------------------------------*/
extern int rtkpos(FILE* fp_trace, FILE* fp_table, rtk_t* rtk, const obsd_t* obs, int nobs, const nav_t* nav)
{
    prcopt_t *opt=&rtk->opt;
    sol_t solb={{0}};
    gtime_t time;
    ...
    trace(NULL,5,"obs=\n"); tracetable(fp_table,5,obs,nobs);

    for (i = 0; i < nobs; i++) {
        index = obs[i].sat - 1;
        for (j = 0; j < NFREQ; j++) {
            if (abs(obs[i].L[j] - 0) > 1E-3) rtk->hasL[index][j] = 1;
            if (abs(obs[i].P[j] - 0) > 1E-3) rtk->hasP[index][j] = 1;
        }
    }
    ...
    if (!pntpos(fp_trace,obs,nu,nav,&rtk->opt,rtk,&rtk->sol,NULL,rtk->ssat,msg)) {
    ...
    trace(NULL, 2, "%s,point position,%14.4f\t%14.4f\t%14.4f\n", time_str(obs[0].time, 3), rtk->sol.rr[0], rtk->sol.rr[1], rtk->sol.rr[2]);
    ...
    if (opt->mode==PMODE_MOVEB) { /*  moving baseline */
        ...
        if (!pntpos(NULL,obs+nu,nr,nav,&rtk->opt,NULL,&solb,NULL,NULL,msg)) {
    ...
    relpos(fp_trace,rtk,obs,nu,nr,nav);
