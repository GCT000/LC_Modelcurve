...
static void dump_stas(const strfile_t *str)
{
    ...
    for (p=str->stas;p;p=p->next) {
        ...
        /*trace(5,"%s %s  %04d %-6.6s %-16.16s %-16.16s %12.8f %13.8f %9.3f %2d "
                     ...
                     int *mask)
{
    ...
    */
    ...
    for (i=0;i<NSATSYS;i++) {
        ...
        for (j=0;j<n[i];j++) {
           /* trace(5,"scan_file: sys=%d code=%s\n",i,code2obs(codes[i][j]));*/
        }
    }
    /* set station info in RINEX options */
    setopt_sta(str,opt);
...
static void setopt_apppos(strfile_t *str, rnxopt_t *opt)
{
    ...
    if (!pntpos(NULL,str->obs->data,str->obs->n,str->nav,&prcopt,NULL,&sol,NULL,NULL,
                msg)) {
                     ...
                     char **ofile)
{
    ...
    /*trace(5,"convrnx_s: sess=%d format=%d file=%s ofile=%s %s %s %s %s %s %s "
...
extern int convrnx(int format, rnxopt_t *opt, const char *file, char **ofile)
{
    ...
    /*trace(5,"convrnx: format=%d file=%s ofile=%s %s %s %s %s %s %s %s %s\n",
