...
namespace lc_core
{
    ...
    class CurveP2PFactorA : public ceres::SizedCostFunction<1, 1, 1, 1, 1, 1, 1>
    {
    public:
        CurveP2PFactorA(const cv::Point2d &_img_p, const double &_y, const Trans &_Tcl, std::shared_ptr<Camera> _cam, const WeightType &_weight_type = WeightType::Equal)
        ...
        virtual bool Evaluate(double const *const *parameters, double *residual, double **jacobians) const
        {
            ...
            pLidar << a1 * y * y + b1 * y + c1, y, a2 * y * y + b2 * y + c2;
            ...
            if (jacobians)
            {
                if (jacobians[0])
                {
                    Eigen::Map<Eigen::Matrix<double, 1, 1, Eigen::RowMajor>> jacobians_p2a1(jacobians[0]);
                    Eigen::Matrix<double, 3, 1> p2a1;
                    p2a1 << y * y, 0, 0;
                    jacobians_p2a1 = matrix_V2Pimg * matrix_Pimg2Pcam * Tcl.R.cast<double>() * p2a1 * sqrt_info;
                }

                if (jacobians[1])
                {
                    Eigen::Map<Eigen::Matrix<double, 1, 1, Eigen::RowMajor>> jacobians_p2b1(jacobians[1]);
                    Eigen::Matrix<double, 3, 1> p2b1;
                    p2b1 << y, 0, 0;
                    jacobians_p2b1 = matrix_V2Pimg * matrix_Pimg2Pcam * Tcl.R.cast<double>() * p2b1 * sqrt_info;
                }

                if (jacobians[2])
                {
                    Eigen::Map<Eigen::Matrix<double, 1, 1, Eigen::RowMajor>> jacobians_p2c1(jacobians[2]);
                ...
                if (jacobians[3])
                {
                    Eigen::Map<Eigen::Matrix<double, 1, 1, Eigen::RowMajor>> jacobians_p2a2(jacobians[3]);
                    Eigen::Matrix<double, 3, 1> p2a2;
                    p2a2 << 0, 0, y * y;
                    jacobians_p2a2 = matrix_V2Pimg * matrix_Pimg2Pcam * Tcl.R.cast<double>() * p2a2 * sqrt_info;
                }

                if (jacobians[4])
                {
                    Eigen::Map<Eigen::Matrix<double, 1, 1, Eigen::RowMajor>> jacobians_p2b2(jacobians[4]);
                    Eigen::Matrix<double, 3, 1> p2b2;
                    p2b2 << 0, 0, y;
                    jacobians_p2b2 = matrix_V2Pimg * matrix_Pimg2Pcam * Tcl.R.cast<double>() * p2b2 * sqrt_info;
                }

                if (jacobians[5])
                {
                    Eigen::Map<Eigen::Matrix<double, 1, 1, Eigen::RowMajor>> jacobians_p2c2(jacobians[5]);
